...
import type { MilkCollection, MilkShift } from "../../../types/milkCollection.ts";
import type { MilkType } from "../../../types/farmer.ts";
import { getMilkEntries, deleteMilkEntry } from "../../../axios/milk_api.ts";
import toast from "react-hot-toast";
import { getMilkListLocal, saveMilkList } from "../../../utils/offlineData.ts"
...
const MilkCollectionListPage: React.FC = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // ✅ Save for offline
        saveMilkList(res.data);
      } catch (err) {
        console.error("Failed to load milk collections:", err);

        // ✅ Fallback to offline data
        const local = getMilkListLocal();
        setCollections(local);

        toast.error("Offline mode: showing saved data");
      }
    };
  ...
  const columns: DataTableColumn<MilkCollection>[] = [
    ...
    },
    {
      ...
      cell: (row) => (
        <span
          className={`inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-medium ${row.milkType === "cow"
              ...
              <div className="flex gap-2">
                {(["today", "all", "range"] as DateFilterMode[]).map((mode) => (
                  <button
                    ...
                    className={`rounded-md px-3 py-1.5 text-xs font-medium ${dateMode === mode
