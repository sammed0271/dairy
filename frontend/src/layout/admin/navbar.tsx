          ...
          {showSettings && (
            <div className="absolute right-0 mt-2 w-64 sm:w-56 rounded-lg border bg-[#F8F4E3] dark:border-gray-700 shadow-lg z-50">
              ...
              <div className="flex items-center justify-between px-4 py-3  transition">
                ...
                <button
                  onClick={toggleTheme}
                  className={`relative w-12 h-6 flex items-center rounded-full p-1 transition-colors duration-300 ${isDark ? "bg-[#2A9D8F]" : "bg-gray-300"
                    }`}
                >
                  <div
                    className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform duration-300 flex items-center justify-center text-xs ${isDark ? "translate-x-6" : "translate-x-0"
                      }`}
                  >
                    {isDark ? "🌙" : "🌞"}
                  </div>
